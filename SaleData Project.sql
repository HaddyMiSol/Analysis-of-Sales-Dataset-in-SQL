...
select city, productline, SALES, sum(sales) Revenue  , count(ORDERNUMBER) Frequency
from dbo.sales_data_sample
where SALES >= 10
AND SALES <=1000
group by city, productline,SALES
order by 2 desc
...
---what is the most expensive product and what is the frequency of purchase
...
select Top 2 * from
(select Top 5 PRODUCTLINE, PRICEEACH
from dbo.sales_data_sample
order by PRODUCTLINE, PRICEEACH desc) Expensive_Product
order by PRODUCTLINE, PRICEEACH desc

---RANKING FUNCTION
--Assign ranks to the price of each product
with cte as 
(select 
productline,
priceeach,
DENSE_RANK () Over(partition by productline order by PRICEEACH DESC) as Ranking
from dbo.sales_data_sample) 
